...
class Lucifer
  ...
  def send_email(who, which, msg, proc_n="")
    ...
    temp_email = "#{proc_n||'nil'}_#{stamp}.email"
...
if $0 == __FILE__
  config = {
    ...
    :max_cpu=>90,
    :max_mem=>90,
    :process_string=>"/usr/local/bin/ruby /usr/local/bin/thin start --no-epoll --threaded --port 8080 -R config.ru",
    :alert_email=>"alerts@some-email-address.com",
    :start_script=>"cd /usr/share/deploy/platform/MyApp/d2;./myapp-start.sh",
    :restart_email_template=>"./restart.txt",
    :not_running_email_template=>"./not-running.txt",
    :cpu_email_template=>"./emails/cpu-too-high.txt",
    :mem_email_template=>"./mem-too-high.txt",
    :verify_url=>"http://myapp.com:8080/app" # THIS NEEDS TO BE A VALID URL OF THE WEB APP YOU ARE MONITORING
  }
  devil = Lucifer.new(config)
  devil.do_monitor_cycle # note you could get this app to handle its own cycles or use crontab
end
